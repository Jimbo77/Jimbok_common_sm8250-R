...
static int __init devfreq_boost_ddr_init(void)
{
	...
	for (i = 0; i < DEVFREQ_MAX_DDR; i++) {
		struct boost_dev *b = d->devices + i;
		b->wq_i = alloc_workqueue("devfreq_boost_ddr_wq_i", WQ_POWER_EFFICIENT, 0);
		b->wq_f = alloc_workqueue("devfreq_boost_ddr_wq_f", WQ_POWER_EFFICIENT, 0);
		b->wq_m = alloc_workqueue("devfreq_boost_ddr_wq_m", WQ_POWER_EFFICIENT, 0);
